...
class FileProcessingUtils:
    ...
    def stream_csv_batches(self, bucket: str, key: str, batch_size: int = 100) -> Iterator[List[Dict[str, Any]]]:
        ...
        try:
            ...
            for row in csv_reader:
                ...
                cleaned_row = self._optimize_row_data(row)
            ...
            self.logger.error(f"CSV streaming error: {e}")
            raise
        finally:
    ...
    def _optimize_row_data(self, row: Dict[str, Any]) -> Dict[str, Any]:
        """Optimize row data by removing empty values and converting types"""
        optimized = {}
        ...
        for key, value in row.items():
            if value is not None and value != '':
                ...
                if self._is_numeric(value):
                    try:
                        optimized[key] = int(value) if '.' not in value else float(value)
                    except ValueError:
                        optimized[key] = value
                else:
                    optimized[key] = value
    ...
    def get_safe_file_content(self, bucket: str, key: str, fallback_content: str = None) -> str:
        ...
        try:
            ...
            if fallback_content:
                self.logger.warning(f"S3 access failed (using fallback): {e}")
